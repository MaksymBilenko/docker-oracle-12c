...
echo | /u01/app/oracle/product/12.1.0/xe/root.sh || true
...
case "$1" in
	'')
		...
		if [ "$(ls -A /u01/app/oracle/oradata)" ]; then
			...
			su oracle -c 'echo startup\; | $ORACLE_HOME/bin/sqlplus / as sysdba'
		else
		...
		while [ "$END" == '' ]; do
			sleep 1
			trap "su oracle -c 'echo shutdown immediate\; | $ORACLE_HOME/bin/sqlplus / as sysdba'" INT TERM
		done
		;;
